...
export default function CartSection({
  ...
  onAddToCart: (item: Product, type: "decrement" | "increment") => void;
  isLoading: boolean;
  onOrder: () => void;
}) {
  ...
  const handleAddNote = (id: string, notes: string) => {
    setCarts(
      carts.map((item) => (item.product_id === id ? { ...item, notes } : item))
    );
  };
  return (
              ...
              <div key={item.product.id} className="space-y-2">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <Image
                      src={item.product.image_url as string}
                ...
                <div className="flex items-center gap-4 w-full">
                  <Input
                    ...
                    onChange={(e) =>
                      debounce(
                        () => handleAddNote(item.product!.id, e.target.value),
                        500
                      )
                    }
                  />
                  <div className="flex items-center gap-4">
                    <Button
                      ...
                      variant="outline"
                      onClick={() => onAddToCart(item.product!, "decrement")}
                    >
                      -
                    </Button>
                    ...
                    <Button
                      className="font-semibold cursor-pointer"
                      variant="outline"
                      onClick={() => onAddToCart(item.product!, "increment")}
                    >
                      +
                    </Button>
                  </div>
