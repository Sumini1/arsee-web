...
export default function DetailOrder({ id }: { id: string }) {
  ...
  const {
    data: orderProduct,
    isLoading: isLoadingOrderProduct,
    refetch: refetchOrderProduct,
  } = useQuery({
    queryKey: ["orders_product", order?.id, currentPage, currentLimit],
    queryFn: async () => {
      const result = await supabase
        .from("orders_products")
        .select("*, products (id, name, image_url, price)", { count: "exact" })
        .eq("order_id", order?.id)
        .order("status");

      if (result.error)
        toast.error("Get order product data failed", {
          description: result.error.message,
        });
  ...
  const [updateStatusOrderState, updateSatusOrderAction] = useActionState(
  ...
  const handleUpdateStatusOrder = async (data: {
    ...
    startTransition(() => {
      updateSatusOrderAction(formData);
    });
  };
  ...
  const filteredData = useMemo(() => {
    return (orderProduct?.data || []).map((item, index) => {
      return [
  ...
  const totalPages = useMemo(() => {
    return orderProduct && orderProduct.count !== null
      ? Math.ceil(orderProduct.count / currentLimit)
      : 0;
  }, [orderProduct]);
    ...
    <div className="w-full space-y-4 ">
      ...
      <div className="flex flex-col lg:flex-row gap-4 w-full ">
        ...
        <div className="lg:w-1/3 flex flex-col gap-4">
          {order && (
            <Summary order={order} orderProduct={orderProduct?.data} id={id} />
          )}
        </div>
      </div>
    </div>
