...
export async function createOrder(
  ...
  const orderid = `ARSEWEB-${Date.now()}`;
  ...
  if (orderError || tableError) {
    const error = orderError || tableError;
    return {
      status: "error",
      errors: {
        ...prevState.errors,
...
export async function addOrderItem(
  ...
  const supabase = await createClient();
  const payload = data.items.map(({ product, ...item }) => item);
  const { error } = await supabase.from("orders_products").insert(payload);
